function sOut = midlineKinematics_noseTail(videoName, DLTpts, varargin)
...
%% ask a few questions about how you want to process the video
    ...
    prompt = {'Enter number of points to save along midline:'};
    dlgtitle = 'Input';
    dims = [1 35];
    definput = {'21'};
    answer = inputdlg(prompt,dlgtitle,dims,definput);
    npts = str2double(answer{1});
 ...
 %% Fix Circle Fish
    if inner~=0 %if the fish is a circle/donut shape
        ...
        if framesToProcess(Index) == startFrame %if on start frame
            ...
            thresh = round(size(B{1},1)/30); %base cut-off on length of fish boundary
            clear B
        else %if not on start frame
            fishLength = arclength(Lines(Index-1).MidLine(:,1), Lines(Index-1).MidLine(:,2));
            thresh = round(fishLength*0.05); % 0.075 base cut-off on length of fish midline in previous frame. 7.5% since this is ~ half the length of the skull.
        end
        
        [path,skeletonTail,skeletonNose,p_length] = traceFish(Skeleton,...
            branches_and_ends,SkelBranches,Nose,Tail,frame_rows,frame_cols,thresh); %find path along skeleton
        [p_row,p_col] = find(path(:,:)>0);
...
sOut.midLines = Lines;
midlineStruct = sOut;
save([videoName,'_midlines.mat'], 'midlineStruct');
...
function [paths,be_closest_tail,be_closest_head,p_length] = traceFish(skeleton,branches_and_ends,branches,head,tail,frame_rows,frame_cols,thresh)
...
if chk == 1
    if min_dist_head < thresh %If the initial point choosen is too close
        while min_dist_head < thresh %If your closest branchpoint is TOO close
        ...
        if min_dist_head > thresh*2 %If the closest branchpoint larger than the threshold is too far down the body, use the previous branchpoint.
            J=prevJ;
        end
    elseif min_dist_head > thresh*10 %If the initial point choosen is too far away, add in endpoints
        clear b_coordinates
